...
class SSLKeyGenerator:
    ...
    def key_generation(self, os_local):
        ...
        if not os.path.exists(self.Key):
            ...
            try:
                subprocess.run(['openssl', 'req', '-new', '-sha256', '-subj', '/CN=esgi.com', '-key', f'{self.Key}/CERT/cert-key.pem', '-out', f'{self.Key}/CERT/cert-query.csr', '-config', openssl_config_path], check=True)
