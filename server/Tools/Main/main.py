...
def read_ip_addresses():
        ip_server = "N/A"
        ssl_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..','..','..', 'SSL', 'CERT', 'extfile.cnf.txt'))
        
        if os.path.exists(ssl_path):
            with open(ssl_path, 'r') as file:
                for line in file:
                    if 'subjectAltName' in line:
                        ip_server = line.split(':')[1].strip()
...
def main(param):
    ...
    if param == 2:
        port = sys.argv[2]
        host = read_ip_addresses()
        socket_obj = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        socket_obj.bind((host, int(port)))
        socket_obj.listen(5)
        print(f"Serveur Up on {host} listenning on port {port}")
